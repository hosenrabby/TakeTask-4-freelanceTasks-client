...
const Login = () => {
    const { signInWithGoogle, signInWithEmail,explocation } = use(AuthContext);
    ...
    return (
        <>
        <title>Take Task | Sign In</title>
            <div className="min-h-[calc(100vh-200px)] flex items-center justify-center bg-[#101828] px-4 py-8">
                <div className="bg-base-100 rounded-2xl p-8 shadow-lg w-full max-w-md">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            <p className='text-sm text-base-content'>Dint Have an Account <Link to={'/sign-up'} className='link link-primary'>Sign Up</Link> </p>
                        </div>
                        {/* Submit */}
                        <button className='w-full flex justify-center items-center gap-3 p-1 md:p-3 px-3 md:px-5 transition rounded-md bg-base-100 border border-gray-700 hover:bg-[#5BBB7B] hover:text-base-content hover:border-[#5BBB7B] cursor-pointer'>Sign In<GoArrowUpRight /></button>
                    </form>
