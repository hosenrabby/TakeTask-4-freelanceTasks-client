        ...
        <>
            <div className='bg-base-200'>
                <div className=" py-12 px-4 max-w-7xl mx-auto">
                    <motion.h2 className="text-center text-2xl font-bold" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} > Membership Plans </motion.h2>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {basePlans.map((plan, idx) => (
                            <motion.div key={idx} className={`rounded-lg shadow-md p-6 border bg-base-100 relative ${plan.recommended ? "border-[#5BBB7B] shadow-blue-100" : "border-gray-200"
                                }`} whileHover={{ scale: 1.03 }} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} >
                                {plan.recommended && (
                                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#5BBB7B] text-white text-sm rounded">
                                        Recommended
                                    </div>
                                )}
                                ...
                                <div className={`text-3xl font-bold py-3 px-6 mb-4 rounded text-center ${plan.recommended ? "bg-[#5BBB7B] text-base-content" : "bg-base-100 text-base-content"}`}>
                                    {getPrice(plan.price)}
                                </div>
                                ...
                                <button
                                    className={`w-full py-2 rounded border font-medium ${plan.recommended
                                        ? "bg-[#5BBB7B] text-base-content hover:bg-[#1F4B3F]"
                                        : "border-[#5BBB7B] text-base-content hover:bg-[#1F4B3F] hover:text-base-content transition"
                                        }`}
                                >
                                    Buy Now
                                </button>
